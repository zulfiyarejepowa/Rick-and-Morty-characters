...
const renderCards = (apiUrl) => {
    fetch(apiUrl, {method: 'GET'}).then((res) => res.json())
    .then((parseData) => {
    console.log(parseData);
    totalCharacters.textContent = parseData.info.count;

    loadMore.dataset.apiUrl = parseData.info.next;

    parseData.results.forEach(character => {
        const htmlCard = `
        <div class="card">
